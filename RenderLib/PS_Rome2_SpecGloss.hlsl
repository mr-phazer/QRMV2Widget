...
{      
    ...
    //return float4(0,GlossTex.g,  0, 1);
    ...
    float3 rgb = GlossTex.g;
    //return float4(GlossTex.rg, 0, 1);
    float smoothness = GlossTex.g;
    
    float gloss = GlossTex.x * GlossTex.x;
    
    
    float specular_level = GlossTex.y;
...
//    float3 N2 = mul(N, (float3x3) mRotEnv);
   ...
   // float3 Lr = 2.0 * cosLo * N- Lo;  // written out reflect formula
    ...
    for (uint i = 0; i < 1; ++i)
    {
        ...
        float3 specularBRDF = ((F * D * G) / max(Epsilon, 4.0 * cosLi * cosLo)) * specular_level*6;

...
struct readable_and_wite_vector : public ISerializable1, public vector<std::shared_ptr<ISerializable1>>
{
...
struct i_vertex_raw_base
{
	...
	void setInfluences(vector< pair<int, float >> _influences);
...
//{
//public:
//	uint32_t read_size = sizeof(TYPE1) * 4;
//
//	vertex_element_xyzw(element_type _type = element_type::INVALID, text textName = "")
//	{
//		type = _type;
//		size = sizeof(raw);
//		pDestination = &raw;
...
//	vertex->weight1 = 1.0f - vertex->weight;
	...
	bool write(ofstream& out, size_t _size)
	{
		...
		out.write((char*)this, vertex_size);
...
struct IVertexBase : public ISerializable1
{
...
*/
	...
	vector<pair<int, float>> vecInfluences = { {-1, 0.0} };
	//uint32_t lod, groupo;
...
struct RMV2_Vertex_Common1
{

	//o.v = textcords.y; // maybe this; maybe not
	RMV2_Vertex_Common1()
	{
		//uint32_t ui = sizeof(*this);
		//ZeroMemory(this, ui);
		//vecInfluences.clear();
		//vecInfluences.push_back({ -1, 0.0 });

	};

	RMV2_Vertex_Common1(const  RMV2_Vertex_Common1&)
	{
		int a = 1;
		//uint32_t ui = sizeof(*this);
		//ZeroMemory(this, ui);
...
//	vertex->weight1 = 1.0f - vertex->weight;
	...
	/// </summary>
	/// <returns></returns>
	const RMV2_Vertex_Raw_Cinematic& getCinematicRaw();
	...
	static RMV2_Vertex_Raw_Default oDefaultVertex;
	static RMV2_Vertex_Raw_Cinematic oCinematicRaw;
	static RMV2_Vertex_Raw_Weighted otWeightedRaw;
...
struct RMV2_Vertex_Raw_1
{
	uint32_t filler;
	half pos_x, pos_y, pos_z, pos_w;
